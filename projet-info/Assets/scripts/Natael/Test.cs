...
public class NatPathTest : MonoBehaviour
{
    // Variables public
    ...
    public NatPathTest(int largeur, int hauteur)
    {
    ...
    public List<NatNode> FindPath(int startX, int startY, int endX, int endY, GameObject obj1)
    {
        // Analyser la map
        //nodeMap = new Node[grid.GetLargueur(), grid.GetHauteur()];
        NatNode start = FindNode(obj1);
    ...
    public NatNode FindNode(GameObject obj)
    {
        ...
        {
            if (collidingObject.gameObject.GetComponent<NatNode>() != null)
            {
                // Le joueur est sur cette node
                NatNode tile = collidingObject.gameObject.GetComponent<NatNode>();
